...
class CompetitionRateController extends Controller
{
    ...
    private function fetchMixedRates($url)
    {
        ...
        if (empty($dbRates)) {
            Log::warning('No hay data en la DB para la comparativa, devolviendo estructura básica');
            return [
                [
                    'entity' => 'CambiaFX',
                ...
                [
                    'entity' => 'Dólar Ocoña',
                    'buy' => number_format($buyLive - 0.002, 3),
                    'sell' => number_format($sellLive + 0.002, 3),
                    'diff' => '- S/ 0.002',
                    'category' => 'Paralelo'
                ]
            ];
